...
export const CALENDAR_BOTTOM_DETAIL_CONSTRAINT = '画面底部约20%~25%必须自然延续上方场景，用地面、水面、天空、光影、色彩渐变或材质纹理保持画面充实连贯；仅降低细碎元素密度和局部对比，不留空白、不做纯色块或明显分区，确保未叠加日历板时画面仍完整和谐'

export const CALENDAR_LAYOUT_CONSTRAINTS: Record<CalendarAspectRatio, string> = {
    '9:16': '9:16竖幅满版底图，主体位置、景别和尺度可灵活变化，可偏置、出界或局部特写；整体场景连续完整，排版区域通过降低局部对比和细节频率形成，而不是留白或切出空区；' + CALENDAR_BOTTOM_DETAIL_CONSTRAINT + '；画面内不出现文字、数字、日期、日历网格、Logo、水印或二维码。',
    '4:5': '4:5竖幅满版底图，主体位置、景别和尺度可灵活变化，可偏置、出界或局部特写；整体场景连续完整，排版区域通过降低局部对比和细节频率形成，而不是留白或切出空区；' + CALENDAR_BOTTOM_DETAIL_CONSTRAINT + '；画面内不出现文字、数字、日期、日历网格、Logo、水印或二维码。'
}
