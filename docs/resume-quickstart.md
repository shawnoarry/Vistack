...
本文使用 `D:\github` 作为示例工作区。如果新机器实际放在其他磁盘或目录，必须把本文命令和启动指令中的 `D:\github` 替换为真实绝对路径，不能因为路径不同重新克隆到错误位置。
...
浏览器打开终端显示的地址，Vite 默认通常是 `http://127.0.0.1:5173/`；如端口被占用，以终端实际输出为准。结束时在终端按 `Ctrl+C`。
